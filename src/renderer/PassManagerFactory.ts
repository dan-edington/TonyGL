...
function PassManagerFactory(renderer: Renderer) {
  return function createPassManager(): PassManager {
    ...
    function runPass(name: string, commandEncoder: GPUCommandEncoder): void {
      ...
      if (!scene) throw new Error(errorMessages.missingPassScene);
      if (!camera) throw new Error(errorMessages.missingPassCamera);

      pass.runPass(commandEncoder, scene, camera, buildPassContext(pass));
    }
